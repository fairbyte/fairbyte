...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "fairbyte is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "fairbyte is downloading blocks...");
    /*
	...
	*/
    ...
    if (pindexBest->nHeight > FIRST_POS_BLOCK && pindexBest->nHeight <= MIDDLE_POW_BLOCK)
    	throw JSONRPCError(RPC_MISC_ERROR, "No PoW blocks available for now. Wait for block number 200000.");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "fairbyte is not connected!");
