...
unsigned int nStakeMaxAge = 60 * 60 * 24 * 120;	    // stake age of full weight: 120d
...
const string strMessageMagic = "fairbyte Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // fairbyte: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
            	if (pwallet->IsFromMe(tx))
...
bool AddOrphanTx(const CTransaction& tx)
{
    ...
    // 20,000 orphans, each of which is at most 5,000 bytes big is
    // at most 1000 megabytes of orphans:
                        ...
                        bool* pfMissingInputs)
{
    ...
    // fairbyte: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees, const CBlockIndex* pindex)
{
	...
	if((nHeight > FIRST_POS_BLOCK && nHeight <= MIDDLE_POW_BLOCK) || nHeight > LAST_POW_BLOCK)
			return 0;
...
static const int64_t ifac1 = 1000;
static const int64_t ifac2 = 100;
static const int64_t ifac3 = 5;
static const int64_t ifac4 = 2;
...
int64_t GetProofOfWorkBonusRewardFactor(CBlockIndex* pindex)
{
	...
	if(pindex->nHeight < 3 || ((pindex->nHeight > FIRST_POS_BLOCK && pindex->nHeight <= MIDDLE_POW_BLOCK) || pindex->nHeight > LAST_POW_BLOCK))
		return 0;
	...
	if(random > 47000 && random < 47011)		// 1/21600
	{
		return ifac1;
	}
	else if(random > 97100 && random < 97201)	// 1/2160
	{
		return ifac2;
	}
	else if(random > 122000 && random < 123001)	// 1/216
	{
		return ifac3;
	}
	else if(random > 173000 && random < 180201)	// 1/30
	{
		return ifac4;
	}
	return 0;
...
// fairbyte: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // fairbyte: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // fairbyte: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // fairbyte: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // fairbyte: track money supply and mint amount info
...
// fairbyte: total coin age spent in transaction, in the unit of coin-days.
...
// fairbyte: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // fairbyte: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // fairbyte: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // fairbyte: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // fairbyte: compute stake modifier
    ...
    {
        /*return error("AddToBlockIndex() : Rejected by stake modifier checkpoint height=%d, modifier=0x%016"PRIx64, pindexNew->nHeight, nStakeModifier, pindexNew->nStakeModifierChecksum);*/
...
bool CBlock::AcceptBlock()
{
    ...
    if (IsProofOfWork() && ((nHeight > FIRST_POS_BLOCK && nHeight <= MIDDLE_POW_BLOCK) || nHeight > LAST_POW_BLOCK) )
    ...
    // fairbyte: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // fairbyte: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // fairbyte: getblocks may not obtain the ancestor block rejected
    ...
    // fairbyte: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "fairbyte", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "fairbyte x13 2014-06-25 14:35:23";
        CTransaction txNew;
        txNew.nTime = 1403687659;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1403687659;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 3624442;
        if(fTestNet)
        {
            block.nNonce = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x061a9270c582e044540b2cabbf437f9867d4104c1b1210ec2b5cccc7be3c71c7"));
        block.print();
        ...
        // fairbyte: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // fairbyte: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // fairbyte: tell downloading node about the latest block if it's
